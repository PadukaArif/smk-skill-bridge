        ...
        <main className=' p-8 bg-white text-neutral-800 w-[75dvw] rounded-4xl shadow-2xl m-8 mx-auto'>
            ...
            <section className=' flex flex-col gap-4 justify-content-center align-items-center'>
                ...
                {dataQuestion?.questions.map((a) => {
                    const currSelect = document.getElementById(`pertanyaan${a.no}`) as HTMLSelectElement
                    return (
                        <div className=" flex gap-2 flex-col p-2 shadow rounded-2xl border border-neutral-300" key={a.no}>
                            <label htmlFor="" className=' text-xl'>{a.no}. {a.question}</label>
                            <select name="" id={`pertanyaan${a.no}`} className=' border p-2 rounded-lg shadow' onChange={(e) => setAnswerArray(e)}>
                                <option defaultValue="default">Pilih jawaban</option>
                                {a.options.map(o => {
                                    return (
